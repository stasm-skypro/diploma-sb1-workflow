...
def test_list_reviews(auth_client, review):
    ...
    """
    url = reverse("bulletin:reviews-list")
...
def test_retrieve_review(auth_client, review):
    """
    Тестирует получение отзыва по ID.

    - Отправляет GET-запрос на /reviews/<id>/
    - Проверяет, что статус ответа — 200 OK
    - Проверяет соответствие данных отзыва

    :param auth_client: Аутентифицированный клиент
    :param review: Объект отзыва
    """
    url = reverse("bulletin:reviews-detail", args=[review.id])
    response = auth_client.get(url)
...
def test_create_review(auth_client, bulletin):
    ...
    assert Review.objects.filter(text="Новый отзыв", bulletin=bulletin).exists()
...
def test_partial_update_review(auth_client, review):
    """
    ...
    """
    url = reverse("bulletin:reviews-detail", args=[review.id])
    data = {"text": "Частично обновлённый текст"}

    response = auth_client.patch(url, data)

    assert response.status_code == status.HTTP_200_OK
    review.refresh_from_db()
    assert review.text == "Частично обновлённый текст"
